...
const renderFilms = () => {
    ...
    if (filteredFilms.length === 0) {
      ...
      <div class="mb-3 d-flex align-items-center flex-wrap gap-2">
        ...
        <button id="btn-available-films" class="btn btn-primary">Ingresos por tiendas</button>
        
        <div class="input-group" style="width: 350px;">
          <input type="text" id="search-input" class="form-control" placeholder="Buscar por título o descripción...">
          <button class="btn btn-primary" type="button" id="filter-button">Filtrar</button>
            
          
          <button class="btn btn-outline-secondary" type="button" id="clear-search">Limpiar</button>
  ...
  <div class="mb-4 d-flex align-items-center flex-wrap gap-3">
    ...
    <span class="badge bg-primary ms-2">${filteredFilms.length} resultados</span>
  </div>
`;
    const table = `
  ${buttons}
...
const renderPagination = () => {
    ...
    for (let i = startPage; i <= endPage; i++) {
        buttons += `<button class="btn ${i === currentPage ? "btn-dark" : "btn-outline-dark"} mx-1 rounded-pill" data-page="${i}">${i}</button>`;
    }
